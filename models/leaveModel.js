...
import runInTransaction from '../utility/transactionDButils.js';
...
const LeaveModel = {
  ...
  getUserLeaveBalanceDetails: async (conn, EmployeeCode) => {
    try {
        ...
        const result = await conn
          .request()
          .input('EmployeeCode', mssql.VarChar, EmployeeCode)
          .query(leaveBalanceQuery);

        if (result.recordset.length === 0) {
            // Set Remaining to 0 and leaveType to null
            const emptyResult = [{
                LeaveType: null,
                Remaining: 0
            }];
